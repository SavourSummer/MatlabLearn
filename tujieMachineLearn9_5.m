...
%分类器训练
d=ceil(2*rand); %随机生成维度 ，ceil为取整
[xs,xi]=sort(x(:,d));
el=cumsum(y(xi));%cumsum（cumulative sum）计算累积和。el(i) 表示 y(xi(1)) + y(xi(2)) + ... + y(xi(i))。这个累积和在后续的步骤中用于计算误差
eu=cumsum(y(xi(end:-1:1)));%eu(i) 表示 y(xi(end)) + y(xi(end-1)) + ... + y(xi(end-i+1))
